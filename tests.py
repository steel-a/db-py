...
db = DB(Config('getWebData.ini').config['connectionString'])
...
dic = db.getRowDic('select * from test where a=1')
assert dic['a']==1
assert dic['b']=='aaa'

lst = db.getListRowsDic('select * from test')
assert lst[0]['a']==1
assert lst[0]['b']=='aaa'
assert lst[1]['a']==2
assert lst[1]['b']=='bbb'

dic = db.getDicRowsList('select * from test')
assert dic['a'][0]==1
assert dic['a'][1]==2
assert dic['b'][0]=='aaa'
assert dic['b'][1]=='bbb'
